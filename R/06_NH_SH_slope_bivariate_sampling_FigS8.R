...
if (!file.exists(slope_path)) {
  stop(
    ...
    "Run 02_LDG_slope_per_cell.R first:\n",
    slope_path
  )
}
...
paired_df <- LDG_slope %>%
  ...
  pivot_wider(
    names_from = hemisphere,
    values_from = c(slope, k_median, k_cv,k_mean),
    names_sep = "_"
  ) %>%
  ...
  mutate(
    ...
    k_cv_diff = abs(k_cv_Northern - k_cv_Southern),#abs(log((k_cv_Northern + 1) / (k_cv_Southern + 1))),
    k_cv_diff_signed = k_cv_Northern - k_cv_Southern,
    k_cv_asym = abs(log((k_cv_Northern + 1) / (k_cv_Southern + 1))),
    ...
    k_mean_diff = abs(k_mean_Northern - k_mean_Southern),
    k_mean_asym = abs(log((k_mean_Northern + 1) / (k_mean_Southern + 1))),
    k_mean_diff_signed = k_mean_Northern - k_mean_Southern,
...
cor_results <- bind_rows(
  ...
  # cor_one(paired_df, "k_cv_asym", "slope_diff_abs", "|NH-SH slope| vs k_cv asymmetry"),
  cor_one(paired_df, "k_median_diff", "slope_diff_abs", "|NH-SH slope| vs absolute k_median difference"),
  cor_one(paired_df, "k_median_asym", "slope_diff_abs", "|NH-SH slope| vs k_median asymmetry"),
  # cor_one(paired_df, "k_mean_diff", "slope_diff_abs", "|NH-SH slope| vs absolute k_mean difference"),
  # cor_one(paired_df, "k_mean_asym", "slope_diff_abs", "|NH-SH slope| vs k_mean asymmetry"),
  ...
  # cor_one(paired_df, "k_mean_diff_signed", "slope_diff_signed", "NH-SH slope vs signed k_mean difference")
) %>%
  mutate(
    method_group = method_use,
    slope_metric = metric_use,
...
#    Same data basis and plotting structure as 02b
# -----------------------------------------------------------------------

draw_sampling_dissim_time_series <- function(
    sampling_df
) {
  ...
  if (
    ...
    message(
      "Skip sampling-profile dissimilarity ",
      "time series: fewer than 3 non-NA rows."
    )
